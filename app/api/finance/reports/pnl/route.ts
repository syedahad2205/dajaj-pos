...
export async function GET(request: Request) {
  try {
    ...
    try {
      ...
      // ── "Estimated" KPI set — the REAL numbers above, PLUS a projection
      // for whatever Zomato/Swiggy revenue is still sitting in NOT-YET-
      // SETTLED days. It never touches a day that's already settled — for
      // those, Est is byte-for-byte identical to Real (same gross revenue,
      // same Settlement Deduction expense already included in totalExpense
      // above). So:
      //   estimatedTotalRevenue = totalRevenue + Σ(pending day's estimated
      //     net take-home, i.e. displayNet, for days NOT yet settled)
      //   estimatedTotalExpense = totalExpense, UNCHANGED — a pending
      //     settlement hasn't posted a real deduction to the ledger yet, so
      //     there's nothing to add, and nothing already-counted to exclude.
      // This guarantees Est is never LOWER than Real (it only ever adds a
      // projection on top) and the two Expense totals always match exactly
      // — the only thing "Estimated" changes is including a forecast for
      // money that hasn't been settled yet.
      const pendingZomato = roundCurrency(
        lockedClosingsWithSettled.reduce((s, c) => s + (c.zomatoMode === "actual" ? 0 : c.zomatoDisplayRevenue), 0),
      );
      const pendingSwiggy = roundCurrency(
        lockedClosingsWithSettled.reduce((s, c) => s + (c.swiggyMode === "actual" ? 0 : c.swiggyDisplayRevenue), 0),
      );
      const estimatedTotalRevenue = roundCurrency(totalRevenue + pendingZomato + pendingSwiggy);
      const estimatedTotalExpense = totalExpense;
