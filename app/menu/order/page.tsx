...
function DeliveryMenuExperience() {
  ...
  if (!authenticated || role !== "customer") return null;
    ...
    <main className="min-h-screen w-full overflow-x-hidden bg-[linear-gradient(180deg,#fff8ed_0%,#ffe7cf_100%)] py-4 text-slate-900 md:pt-[72px]">
      ...
      <div className="mx-auto w-full max-w-[1200px] px-4 pb-36 md:pb-24">
        ...
        <section className="space-y-4">
          {status ? (
            <div className="rounded-[28px] border border-orange-200 bg-white px-6 py-14 text-center text-sm font-medium text-slate-600">{status}</div>
          ) : categories.length > 0 ? (
            categories.map((category) => {
              ...
              return (
                <section key={category.id} className="rounded-[28px] border border-orange-100 bg-white/90 shadow-sm">
                  <button type="button" onClick={() => setExpandedCategoryId((prev) => (prev === category.id ? null : category.id))} className="flex w-full items-center justify-between gap-4 px-5 py-5 text-left">
                    <div>
                      <h2 className="break-words text-2xl font-black text-slate-900">{isExpanded ? "▼" : "►"} {category.name}</h2>
                      <p className="mt-1 text-sm font-medium text-slate-500">{visibleVariants.length} {visibleVariants.length === 1 ? "item" : "items"}</p>
            ...
            <div className="rounded-[28px] border border-slate-200 bg-white px-6 py-14 text-center text-sm text-slate-500">No categories available.</div>
          )}
        </section>
      </div>

      <button type="button" onClick={() => setCartOpen(true)} className="fixed bottom-20 right-4 z-30 rounded-full bg-slate-900 px-5 py-4 text-sm font-semibold text-white shadow-[0_16px_32px_rgba(15,23,42,0.28)] md:bottom-4">
        Cart ({itemCount})
      </button>
